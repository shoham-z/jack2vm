...
impl SymbolTable {
    ...
    pub fn define(&mut self, name:String, data_type:String, kind:Kind) {
        let index:usize;
        match kind{
            Kind::STATIC => {index = self.static_index;self.static_index +=1}
            Kind::FIELD => {index = self.field_index;self.field_index +=1}
            Kind::ARG => {index = self.arg_index;self.arg_index +=1}
            Kind::VAR => {index = self.arg_index;self.var_index +=1}
