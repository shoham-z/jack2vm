...
pub fn tokenizer(xml_file_path: String) {
    ...
    //reading the data and *it has to be owned otherwise regex will not be able to use it*:
    let file_raw_data:String = fs::read_to_string(xml_file_path.to_string()).unwrap().as_str().to_owned();
    //non readable data that's way next line i transferred it to chars:
    let mut after_no_comments:Cow<str> = regex_no_comments.replace_all(&file_raw_data, "");
    //a vectors for all the chars:
    let mut chars_vec:Vec<char> = vec![];
    for text in file_raw_data.chars() {
        if text != '\n' || text != ' ' {
            chars_vec.push(text);
        }
    }
    let length = chars_vec.len()-1;
