...
impl VMWriter {
    ...
    fn write_push(&mut self, segment:String, index:usize){
        self.vm_file.write(format!("push {} {}\n", segment, index).as_ref()).expect("write push failed");
    }

    /// Writes a VM pop command
    fn write_pop(&mut self, segment:String, index:usize){
        self.vm_file.write(format!("push {} {}\n", segment, index).as_ref()).expect("write pop failed");
    }

    /// Writes a VM arithmetic-logical command
    fn write_arithmetic(&mut self, command:usize){
        match command {
    ...
    fn write_call(&mut self, name:String, n_args:usize){
        self.vm_file.write(format!("call {} {}\n", name,n_args).as_ref()).expect("write if-goto failed");
    }

    /// Writes a VM function command
    fn write_function(&mut self, name:String, n_args:usize){
